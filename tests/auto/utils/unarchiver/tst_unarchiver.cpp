...
private slots:
    ...
    void tst_truncated_archive_fails()
    {
        ...
        write_raw_archive(
            a, archive, {{"big.bin", QByteArray(64 * 1024, 'x'), AE_IFREG, 0644, {}}});
    ...
    // A sparse entry declares a size independent of the data it carries, so a
    // budget counting only the bytes copied out does not see it: 5 GiB of file
    // arrives in a 3.5 kB archive.
    void tst_sparse_entry_is_bounded()
    {
        struct archive *a = archive_write_new();
        archive_write_add_filter_none(a);
        archive_write_set_format_pax(a);

        const FilePath archive = FilePath::fromString(tempDir.path() + "/sparse.tar");
        QCOMPARE(archive_write_open_filename_w(a, archive.path().toStdWString().c_str()),
                 ARCHIVE_OK);

        const qint64 declared = 5LL * 1024 * 1024 * 1024;
        struct archive_entry *entry = archive_entry_new();
        archive_entry_set_pathname_utf8(entry, "sparse.bin");
        archive_entry_set_filetype(entry, AE_IFREG);
        archive_entry_set_perm(entry, 0644);
        archive_entry_set_size(entry, declared);
        archive_entry_sparse_add_entry(entry, declared - 8, 8);
        QCOMPARE(archive_write_header(a, entry), ARCHIVE_OK);
        QCOMPARE(archive_write_data(a, "01234567", 8), la_ssize_t(8));
        QCOMPARE(archive_write_finish_entry(a), ARCHIVE_OK);
        archive_entry_free(entry);
        QCOMPARE(archive_write_close(a), ARCHIVE_OK);
        QCOMPARE(archive_write_free(a), ARCHIVE_OK);

        // Guards the test itself: a writer that padded the hole would make the
        // rest of it a test of the disk rather than of the budget.
        QVERIFY2(archive.fileSize() < 64 * 1024, "the sparse hole was written out");

        const FilePath destination = FilePath::fromString(tempDir.path() + "/dest-sparse");
        Unarchiver unarchiver;
        unarchiver.setArchive(archive);
        unarchiver.setDestination(destination);
        unarchiver.start();

        QVERIFY2(!unarchiver.result(), "a 5 GiB sparse entry passed the size budget");
        QVERIFY2(!(destination / "sparse.bin").exists(), "the oversized entry was created");
    }
