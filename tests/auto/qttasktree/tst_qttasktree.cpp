...
// TODO: Check if the QTaskTree's main guard isn't locked when receiving done signal from each task.
...
struct TestData
{
    ...
    std::optional<int> asyncCount = std::nullopt;
    std::optional<MessageLog> messageLog = std::nullopt;
};
...
void tst_TaskTree::runtimeCheck()
{
    {
...

    {
        ...
        QTest::newRow("TaskTreeRunner")
                << RunnerData{{{"keyA", recipe_A}, {"keyB", recipe_B}, {"keyA", recipe_C}},
                              singleLog, sequentialLog, parallelLog, mappedLog};
    }
}

void tst_TaskTree::taskTreeRunner()
{
...
public:
    ...
    void start() {
        QTimer::singleShot(0, this, [this] {
            Q_EMIT tick();
            QTimer::singleShot(m_interval, this, [this] { Q_EMIT done(DoneResult::Success); });
        });
    }
                                    ...
                                    const StoredBarrierType &barrier, int taskId)
{
    return TickAndDoneTask([storage, barrier, taskId](TickAndDone &tickAndDone) {
        ...
        QBarrier *activeBarrier = barrier.activeStorage();
...
void tst_TaskTree::testTree_data()
{
    ...
    const auto createSync = [storage](int taskId) {
        return QSyncTask([storage, taskId] { storage->m_log.append({taskId, Handler::Sync}); });
    };
    const auto createSyncWithTweak = [storage](int taskId, DoneResult result) {
        return QSyncTask([storage, taskId, result] {
    ...
    {
        auto setupSubTree = [storage, createSuccessTask](QTaskTree &taskTree) {
...

    {
        ...
        const auto barrierKicker = [storage](int taskId) {
            return [storage, taskId](const QStoredBarrier &barrier) {
        ...
        const Group rootWhenSignalDo {
            ...
            When (TickAndDoneTask(onTickSetup), &TickAndDone::tick) >> Do {
                groupSetup(2),
                QSyncTask([storage] { storage->m_log.append({1, Handler::BarrierAdvance}); }),
                createSuccessTask(2),
                createSuccessTask(3)
            }
        };
    ...
    {
        // Test QCustomTask::withTimeout() combinations:
...

    {
        ...
        const Group rootSequentialSuccess = For (RepeatIterator(2)) >> Do {
            sequential,
            successItems
        };
        ...
        const Group rootParallelSuccess = For (RepeatIterator(2)) >> Do {
            parallel,
            successItems
        };
        ...
        const Group rootSequentialError = For (RepeatIterator(2)) >> Do {
            sequential,
            errorItems
        };
    ...
    {
        // Check if task tree finishes with the right progress value when UntilIterator(false).
        const Group root = For(UntilIterator([](int) { return false; })) >> Do {
            storage,
            createSuccessTask(1)
        };
    ...
    {
        // Check if task tree finishes with the right progress value when nested UntilIterator(false).
        const Group root = For (UntilIterator([](int index) { return index < 2; })) >> Do {
            storage,
            For (UntilIterator([](int) { return false; })) >> Do {
                createSuccessTask(1)
            }
        };
...

    {
        ...
        {
        public:
            void operator()(bool *, QTaskInterface *iface) { iface->reportDone(DoneResult::Error); }
        };

        using DummyTask = QCustomTask<bool, DummyTaskAdapter>;
...

    {
        ...
        const auto createRoot = [storage, groupDone](DoneWith doneWith, CallDoneFlags callDone) {
            return Group {
                ...
                parallel,
                Group {
                    TestTask({}, toDoneResult(doneWith == DoneWith::Success)),
                    groupDone(1, callDone)
                },
                doneWith == DoneWith::Cancel ? QSyncTask([] { return false; }) : nullItem
            };
        };

        const Log logNoDone {};
        const Log logSuccess {{1, Handler::GroupSuccess}};
        const Log logError {{1, Handler::GroupError}};
...

    {
        ...
        const ListIterator iterator(tasks);
...

    {
        const Group root {
            ...
            If (createFailingTask(0)) >> Then {
                createSuccessTask(1)
            } >> ElseIf (createFailingTask(2)) >> Then {
                createSuccessTask(3)
            }
        };
...

    {
        const Group root {
            ...
            If (createFailingTask(0)) >> Then {
                createSuccessTask(1)
            } >> ElseIf (createSuccessTask(2)) >> Then {
                createSuccessTask(3)
            }
        };
...
void tst_TaskTree::testTree()
{
    ...
    QTaskTree taskTree({testData.root.withTimeout(1s)});
...
void tst_TaskTree::testInThread_data()
{
    QTest::addColumn<TestData>("testData");
    QTest::newRow("StorageShadowing") << storageShadowingData();
    QTest::newRow("Parallel") << parallelData();
...
void tst_TaskTree::testInThread()
{
    QFETCH(TestData, testData);
...
static std::atomic_bool s_jobStarted = false;
static std::atomic_bool s_treeFinished = false;
static std::atomic_bool s_syncDone = false;
...
static void autoSyncOff(QPromise<void> &promise)
{
...
void tst_TaskTree::testAutoDelayedSync()
{
    ...
    const Group awaiter = For (UntilIterator(isNotStarted)) >> Do {
        timeoutTask(1ms, DoneResult::Success)
    } && errorItem;
    ...
    const DoneWith resultOn = QTaskTree::runBlocking(recipeAutoSyncOn);
    ...
    const DoneWith resultOff = QTaskTree::runBlocking(recipeAutoSyncOff);
    QCOMPARE(resultOff, DoneWith::Error);
    QCOMPARE(s_syncDone, true);
}
...
void tst_TaskTree::storageIO_data()
{
...
void tst_TaskTree::storageIO()
{
    ...
    QTaskTree taskTree(inputOutputRecipe(storage));
...
void tst_TaskTree::storageInitialization_data()
{
...
void tst_TaskTree::storageInitialization()
{
    ...
    QTaskTree taskTree({ storage, onGroupSetup(onSetup) });
    taskTree.runBlocking();
...
void tst_TaskTree::nestedBrokenStorage()
{
    ...
    const auto onOuterSync = [storage, &outerStorage, &innerStorage1, &innerStorage2] {
        ...
        const Group innerRecipe {
            Group {
                ...
                // storage,
                QSyncTask(onInnerSync1)
            },
            Group {
                storage, // Subrecipe OK, another instance for the nested storage will be created.
                QSyncTask(onInnerSync2)
            }
        };

        QTaskTree::runBlocking(innerRecipe);
    };

    const Group outerRecipe {
        storage,
        QSyncTask(onOuterSync)
    };

    QTaskTree::runBlocking(outerRecipe);
...
void tst_TaskTree::restart()
{
    QTaskTree taskTree({TestTask([](TaskObject &taskObject) { taskObject = 1000ms; })});
    taskTree.start();
...
public:
    ...
    void operator()(int *, QTaskInterface *iface) { m_iface = iface; }
private:
    QTaskInterface *m_iface = nullptr;
};

using BrokenTask = QCustomTask<int, BrokenTaskAdapter>;

void tst_TaskTree::destructorOfTaskEmittingDone()
{
    QTaskTree taskTree({BrokenTask()});
    taskTree.start();
}

void tst_TaskTree::restartTaskTreeRunnerFromDoneHandler()
{
...
void tst_TaskTree::validConditionalConstructs()
{
    ...
    // Replace ">>" with "," after the first "Then" to construct 2 independent if conditions.
    Group {
        If (condition) >> Then {
            continuation
        } >> If (condition) >> Then {
...
void tst_TaskTree::exactHandlers()
{
