...
def selectFromFileDialog(fileName, waitForFile=False, ignoreFinalSnooze=False):
    ...
    try:
        ...
        # no line edit (atm?) - click through path
        sideView = "{type='Utils::SidebarView' unnamed='1' visible='1' window=%s}" % qcFileDialog
        fileTree = "{type='QTreeView' unnamed='1' visible='1' window=%s}" % qcFileDialog
        ...
        for p in parts:
            ...
            waitForObject(fileTree).scrollTo(index)
            doubleClick(index)
        # and the file node
        idxStr = "{container=%s text='%s' type='QModelIndex'}" % (fileTree, fName)
        index = waitForObject(idxStr, 1000)
        waitForObject(fileTree).scrollTo(index)
        doubleClick(index)
    except:
        t,v = sys.exc_info()[:2]
        test.log("%s (%s)" % (str(t), str(v)))
        try:
