...
class AndroidManifestIndenter : public TextEditor::TextIndenter
{
public:
    explicit AndroidManifestIndenter(TextEditor::TextDocument *doc) :
             TextEditor::TextIndenter(doc->document()) { }
    ~AndroidManifestIndenter() override = default;
    ...
    int indentFor(const QTextBlock &block, const TextEditor::TabSettings &tabSettings,
                  int cursorPositionInEditor = -1) override
    {
        Q_UNUSED(cursorPositionInEditor);
...
public:
    ...
    {
        TextDocument::setId(Constants::ANDROID_MANIFEST_EDITOR_ID);
        setMimeType(QLatin1String(Constants::ANDROID_MANIFEST_MIME_TYPE));
        TextDocument::setCompletionAssistProvider(
    ...
    : TextEditorWidget(parent)
{
    ...
    textDocument()->setIndenter(new AndroidManifestIndenter(textDocument()));
