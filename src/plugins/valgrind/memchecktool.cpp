...
void MemcheckTool::setupRunControl(RunControl *runControl)
{
    ...
    connect(runControl, &RunControl::aboutToStart, this, [this] {
        ...
        PerspectivesView::instance()->showPermanentStatusMessage(Tr::tr("Starting Memory Analyzer..."));
    });
    connect(runControl, &RunControl::started, this, [] {
        PerspectivesView::instance()->showPermanentStatusMessage(Tr::tr("Memory Analyzer running..."));
    });
...
void MemcheckTool::loadXmlLogFile(const QString &filePath)
{
    ...
    if (!logFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        if (!m_exitMsg.isEmpty())
            PerspectivesView::instance()->showPermanentStatusMessage(m_exitMsg);
        return;
    }
