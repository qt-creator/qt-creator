                                                             ...
                                                             const QVariantMap &defaultData) const
{
    ...
    if (targetAbi.osFlavor() == Abi::AndroidLinuxFlavor) {
        ...
        if (toolchain == "clang-cl") {
            ...
            const auto clangClToolchain = static_cast<ProjectExplorer::ClangClToolchain *>(mainTc);
            data.insert(QLatin1String(CPP_VCVARSALLPATH), clangClToolchain->varsBat().toUrlishString());
