...
{
    m_traceManager->perfLoaders.append({PerfEventType::allFeatures(),
...
static QString displayNameForThread(const PerfProfilerTraceManager *traceManager,
...
void PerfTimelineModelManager::finalize()
{
    ...
    for (auto it = m_unfinished.begin(), end = m_unfinished.end(); it != end; ++it) {
        ...
        if (thread.enabled) {
            model->setDisplayName(displayNameForThread(m_traceManager, thread));
            model->finalize();
    ...
    for (const PerfProfilerTraceManager::Thread &remaining : threads) {
        ...
        model->setDisplayName(displayNameForThread(m_traceManager, remaining));
        model->finalize();
        model->setSamplingFrequency(frequency);
        finished.append(model);
    }
